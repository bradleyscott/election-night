...
| `HEALTH_PORT`                   | `3459`                                                 | Port for the collector's health/live-state JSON endpoint                                                                                                                                                                                   |
| `HISTORY_TOKEN`                 | —                                                      | Collector: bearer token for the `/history/*` REST API. Loopback callers are always trusted; remote callers need this (unset = remote requests 404)                                                                                         |
...
| `HISTORY_UPSTREAM`              | `http://127.0.0.1:3459`                                | Dashboard server: base URL of the collector's history REST API — the server's _only_ source of history data. Default suits a co-located collector; point it at the homelab collector in split deployments (see `deploy/homelab/README.md`) |
| `HISTORY_TOKEN`                 | —                                                      | Dashboard server: bearer token for non-loopback `HISTORY_UPSTREAM` calls; must match the collector's `HISTORY_TOKEN`                                                                                                                       |
...
The container bundles the dashboard, starts the web server, and then starts the scraper (see `deploy/entrypoint.sh`). Set `RUN_COLLECTOR=false` to run the web server only.
...
`Dockerfile.collector` builds a collector-only image for deployment on a residential connection (e.g. Coolify on a Proxmox LXC). It serves a health/live-state endpoint on port 3459, plus the bearer-token-protected `/history/*` REST API which the cloud dashboard server reads via `HISTORY_UPSTREAM` so the Trends page works across the split deployment — see `deploy/homelab/README.md` and `deploy/homelab/Caddyfile` for the full setup.
