...
- `packages/dashboard/src/pages/Flipped.tsx` — `/flipped`: seats whose prior-cycle winner's party is not leading tonight, with the historical margin of victory beside the live lead/MoE and status. Consumes `/api/history/prior-winners`, which arrives already matched to the current cycle (seats with no comparable prior holder are simply absent). `packages/dashboard/src/components/PastWinners.tsx` renders the same data per electorate.
