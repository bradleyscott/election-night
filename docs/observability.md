...
| `election_disk_size_bytes`                   | Gauge     | `path`                |
| `election_disk_available_bytes`              | Gauge     | `path`                |
| `election_build_info`                        | Gauge     | `version`, `revision` |
...
| P0       | Disk exhaustion was invisible until it was unrecoverable      | `election_disk_size_bytes` / `election_disk_available_bytes` sample the DB filesystem every cycle, so the ratio warns before SQLite starts failing. The write counter's `status="error"` was already emitted but had no alert; it now has one. Growth is bounded by `RETENTION_HOURS` (see §4). When the volume did fill, every write failed with `SQLITE_FULL` and the only user-visible symptom was empty pages — the collector kept fetching happily. |
...
- `packages/collector/src/results.ts`, `index.ts` — webhook duration and
  snapshot-write/db-write timing, plus a disk sample (`recordDiskUsage`) at the
  start of each cycle.
- `packages/collector/src/db.ts` — `pruneSnapshots()`: `RETENTION_HOURS` of
  snapshots are kept (24 by default, `0` disables), pruned in batches of 100
  with a WAL checkpoint between them so a nearly-full volume can still make
  progress. `enableIncrementalVacuum()` puts the database in `auto_vacuum =
INCREMENTAL` (a one-time `VACUUM` for a file created before this existed), and
  `compactDatabase()` returns free pages at the end of the file to the
  filesystem — without it, pruning only recycles pages and the file keeps its
  high-water mark forever.
- `packages/collector/src/retention.ts` — the policy: when to sweep
  (`RETENTION_SWEEP_MS`, 15 minutes by default) and how much to keep. A volume
  below 5% free abandons the configured window for one hour
  (`effectiveKeepHours`), because a database that cannot be written serves
  nothing at all. A production collector is expected to run for weeks, so the
  sweep runs for the life of the process — a startup-only prune would fire once
  per deploy and never again.
...
| Database volume filling up        | `election_disk_available_bytes / election_disk_size_bytes < 0.15`                                                                                  | 10m | warning  |
| Database volume nearly full       | `election_disk_available_bytes / election_disk_size_bytes < 0.05`                                                                                  | 5m  | critical |
| Snapshot writes failing           | `increase(election_snapshot_writes_total{status="error"}[10m]) > 0`                                                                                | 5m  | critical |
